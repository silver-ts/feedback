...
const { user, username, handlesignInAnonymously, handleSignInWithGoogle } = useAuth()
</script>

<template>
  <!-- <component :is="user ? (!username ? 'EnterNameView' : null) : 'default'"></component> -->

  <template v-if="user && !username"><EnterNameView /></template>
